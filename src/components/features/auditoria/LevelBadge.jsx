const LEVEL_COLORS = {
    Docente: "bg-[#4CAF501A] text-[#007805] dark:bg-[#16A34A]/20 dark:text-[#5FD68A]",
    Coordenador: "bg-[#022E7C1A] text-[#0040B0] dark:bg-[#1A4A9E]/25 dark:text-[#7FA9F5]",
    Supervisor: "bg-[#FFC1071A] text-[#FFC107] dark:bg-[#D97706]/20 dark:text-[#F0B95B]",
    Administrador: "bg-[#FDE7E81A] text-[#E30613] dark:bg-[#C62834]/25 dark:text-[#F08B92]",
    ADM: "bg-[#FDE7E81A] text-[#E30613] dark:bg-[#C62834]/25 dark:text-[#F08B92]",
};

export default function LevelBadge({ level }) {
    return (
        <span
            className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold align-middle ${
                LEVEL_COLORS[level] || "bg-gray-100 text-gray-700 dark:bg-white/10 dark:text-[#C3C6D3]"
            }`}
        >
            {level}
        </span>
    );
}